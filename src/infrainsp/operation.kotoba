(ns infrainsp.operation
  "InspectionOperationActor -- one road/bridge-infrastructure-
  inspection coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (InspectionAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Infrastructure Inspection Governor (:govern) and the
  ...
  lifecycle is advanced by MANY ops (inspection logging /
  condition-finding decisions / reinspection scheduling), each
  independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human public-works authority/inspection supervisor. The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [infrainsp.advisor :as advisor]
...
(defn build
  ...
  opts:
    :advisor      -- an `infrainsp.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Infrastructure Inspection Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-authority-approval
                                          (:critical? verdict) :critical-reading
